...
const getRandReplicaFromSet = async () => {
  ...
  while (true) {
    if (replicaPbs.length == 0) return pb;
    try {
...
const getMessages = async () => {
  ...
  for (let result of results) {
    const username = (await readPb.collection("users").getOne(result.user))
      .username;
      ...
      try {
        ...
        if (data.code == 200) {
          ...
          await pb.admins.authWithPassword(
            POCKETBASE_USERNAME_AUTH,
            POCKETBASE_PASSWORD_AUTH
          );
          break;
        }
      } catch (e) {
...
const initializePocketBase = async () => {
  ...
  for (let replica of serverList) {
    ...
    await newPb.admins.authWithPassword(
      POCKETBASE_USERNAME_AUTH,
      POCKETBASE_PASSWORD_AUTH
    );
